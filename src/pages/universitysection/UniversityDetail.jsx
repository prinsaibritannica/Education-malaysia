...
const [activeTab, setActiveTab] = useState(
    ...
    const fetchUniversityData = async () => {
      ...
      try {
        ...
        try {
          ...
          fetchedData = response.data?.university || response.data?.data?.university;
        } catch (detailsError) {
          console.log('university-details failed:', detailsError.response?.status, detailsError.response?.data);
          try {
            ...
            fetchedData = response.data?.data?.university || response.data?.university;
          } catch (overviewError) {
            console.log('university-overview failed:', overviewError.response?.status, overviewError.response?.data);
            try {
              ...
              for (const testSlug of possibleSlugs) {
                try {
                  ...
                  const testResponse = await api.get(`/university-details/${testSlug}`);
                  ...
                  console.log(`Slug ${testSlug} failed:`, testError.response?.status);
                }
              }
              ...
              if (found) {
                ...
                window.history.replaceState({}, '', `/university/${found.uname || found.slug || slug}`);
              } else {
                throw new Error(`University "${slug}" not found with any slug variation`);
              }
            } catch (slugTestError) {
              console.log('All slug variations failed, showing static fallback...');
        ...
        console.log('API Response featured_photos:', response.data.featured_photos);
        ...
        if (!fetchedFeaturedPhotos || fetchedFeaturedPhotos.length === 0) {
          console.warn('❌ No featured photos from API - using static photos fallback');
  ...
  offeredCourses: STATIC_OFFERED_COURSES,
  clicks: fetchedData.clicks || "5936",
...
hostel_facility: (() => {
  ...
  if (typeof facility === 'string') {
    ...
    return cleaned ? [cleaned] : ["On-campus accommodation available"];
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center bg-white p-8 rounded-xl shadow-lg max-w-md">
          ...
          <p className="text-gray-600 mb-4">
            The university "{slug}" could not be found or the data is not available.
          </p>
          <Link 
            to="/universities" 
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 mb-8">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-600 mb-6">
                The university "{slug}" was not found in our database. This might be because:
• The university name is spelled incorrectly
• The university is not in our database yet
• The URL slug is different from the university name

Try searching for "Universiti Putra Malaysia" with the correct slug: <code>universiti-putra-malaysia</code>
              </p>
              <Link 
                to="/universities" 
          ...
          {universityData.availableUniversities && universityData.availableUniversities.length > 0 && (
            <div className="bg-white rounded-xl shadow-lg p-8">
              <h2 className="text-xl font-bold text-gray-900 mb-6">Available Universities</h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {universityData.availableUniversities.map((university, index) => (
                  <Link
                    key={index}
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 text-xs text-gray-500">
                          <span className="bg-blue-100 text-blue-700 px-2 py-1 rounded-full">
                            {university.institute_type?.type || 'University'}
                          </span>
                          {university.active_programs_count && (
                            <span>{university.active_programs_count} Programs</span>
                          )}
                        </div>
                      </div>
                    </div>
        ...
        <div className="bg-white p-3">
          ...
          <div className="flex items-start gap-3 mb-4">
            <div className="w-20 h-20 flex-shrink-0 border border-gray-200 rounded-xl p-1">
              <img
                src={universityData?.logo_path ? `${API_URL}${universityData.logo_path}` : '/placeholder-logo.png'}
                alt="University Logo"
...
<div className="flex items-center gap-1 text-gray-600 text-sm">
  ...
  {[...Array(5)].map((_, i) => (
    ...
    />
          ...
          {/* Main Image */}
          <div className="relative mb-4" style={{border: featuredPhotos?.length > 0 ? '3px solid red' : 'none', minHeight: '200px'}}>
...
<button 
  ...
  className="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white px-4 py-2.5 rounded-lg hover:from-blue-600 hover:to-blue-700 shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 text-sm font-bold animate-pulse"
>
...
<div className="bg-white rounded-xl shadow-sm p-5">
  ...
  <ul className="space-y-2 text-sm text-gray-700">
    {Array.isArray(universityData.accredited_by) ? (
      universityData.accredited_by.map((accreditation, index) => (
  ...
  <div className="space-y-4">
    ...
    <div>
      ...
      <div className="w-full bg-gray-200 rounded-full h-2">
        <div className="bg-purple-600 h-2 rounded-full" style={{ width: universityData.local_students && universityData.local_students !== "0" ? '75%' : '0%' }}></div>
      </div>
    </div>
    ...
    <div>
      ...
      <div className="w-full bg-gray-200 rounded-full h-2">
        <div className="bg-blue-600 h-2 rounded-full" style={{ width: universityData.international_students && universityData.international_students !== "0" ? '45%' : '0%' }}></div>
      </div>
    </div>
  </div>
</div>
...
{/* </div> */}
    ...
    {activeTab === "courses" ? (
  <div id="courses-section" className="py-8">
  ...
  <div className="col-span-2 md:col-span-1">
    {/* Get In Touch Form */}
   <div className="mb-2 "> <GetinTouchUiversity /></div>
